...
class NeuralNetwork:
	...
	def __init__(self, learning_rate, training_iteration, batch_size,
		hidden_layer_n, hidden_layer_n2, hidden_layer_n3):
		...
		with tf.name_scope("weights") as scope: #Creating weight matrices and populating them with random numbers
			...
			W3 = tf.Variable(tf.random_normal([hidden_layer_n2, hidden_layer_n3], stddev=0.1))
			W4 = tf.Variable(tf.random_normal([hidden_layer_n3, output_layer_n], stddev=0.1))
		...
		with tf.name_scope("biases") as scope: #Creating bias matrices and populating them with random numbers
			...
			b3 = tf.Variable(tf.random_normal([hidden_layer_n3], stddev=0.1))
			b4 = tf.Variable(tf.random_normal([output_layer_n], stddev=0.1))
		...
		with tf.name_scope("model") as scope: #Creating the three output layers
			...
			layer_3 = tf.nn.sigmoid(tf.matmul(layer_2, W3) + b3)#sigmoid(W[0,0]*i[0] + W[0,1]*i[1] + W[0,2]i[2] + ... + W[0,n]i[0] + b[i])
			layer_4 = tf.nn.softmax(tf.matmul(layer_3, W4) + b4)#softmax(W[0,0]*i[0] + W[0,1]*i[1] + W[0,2]i[2] + ... + W[0,n]i[0] + b[i])
			self.model = layer_4
...
if __name__== "__main__":
	...
	hidden_layer_n2 = 300
	hidden_layer_n3 = 300
	nn = NeuralNetwork(learning_rate, training_iteration, batch_size, hidden_layer_n, hidden_layer_n2, hidden_layer_n3)
	nn.run()
